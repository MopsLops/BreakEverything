execute at @s[tag=air_amulet,tag=!cooldown] run function scr:items/air/air_amulet

execute at @s[tag=water_hoe] run function scr:items/water/hoe/raycast_start

execute at @s[tag=nether_sword,tag=!cooldown] run function scr:items/nether/sword/raycast_start

execute at @s[tag=nether_amulet,tag=!cooldown] run function scr:items/nether/amulet/nether_amulet

execute at @s[tag=ender_staff,tag=!cooldown] run function scr:items/end/staff/ender_staff

execute at @s[tag=ender_artifact,tag=!cooldown] run function scr:items/end/ender_artifact

execute at @s[tag=ender_amulet,tag=!cooldown] run function scr:items/end/amulet/ender_amulet_r

execute at @s[tag=moon_amulet,tag=!cooldown] run function scr:items/moon/timestop

execute at @s[tag=cloud_sword,tag=!cooldown] run function scr:items/cloud/cloud_sword

execute at @s[tag=cloud_stick,tag=!cooldown] run function scr:items/cloud/cloud_stick

execute at @s[tag=cloud_amulet,tag=!cooldown] run function scr:items/cloud/cloud_amulet

execute at @s[tag=spawn_pickaxe,tag=!cooldown] run function scr:items/spawn/spawn_pickaxe